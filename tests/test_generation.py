...
def make_settings(tmp_path: Path, media_mb: int = 1) -> Settings:
    return Settings(uploads_dir=tmp_path / "uploads", derived_dir=tmp_path / "derived", database_path=tmp_path / "db.sqlite", max_upload_size_mb=max(media_mb + 1, 2), max_media_context_size_mb=media_mb)


def make_source(chunk_id: str, *, media_key: str = "", content_modality: str = "image", chunk_text: str = "", file_type: str = "image") -> RetrievedSource:
    return RetrievedSource("doc", chunk_id, f"{chunk_id}.bin", "stored", file_type, "image/png" if media_key else "text/plain", content_modality, 0, chunk_text, chunk_text, media_key, 0.0, 0.9)
...
def test_two_media_below_budget_are_included(tmp_path: Path):
    settings = make_settings(tmp_path)
    sources = [make_source("a", media_key=write_media(settings, "a.png")), make_source("b", media_key=write_media(settings, "b.png"))]
...
def test_aggregate_budget_is_sixty_megabytes(tmp_path: Path, monkeypatch):
    ...
    _, used = _build_contents("pergunta", [make_source("first", media_key=first), make_source("second", media_key=second)], [], "detailed", settings)
    assert used == ["first"]
...
def test_markers_and_parts_are_interleaved(tmp_path: Path):
    settings = make_settings(tmp_path)
    sources = [make_source("image", media_key=write_media(settings, "image.png")), make_source("text", content_modality="text", chunk_text="texto", file_type="text")]
    contents, used = _build_contents("pergunta", sources, [], "detailed", settings)
...
def test_missing_media_is_not_marked_used(tmp_path: Path):
    settings = make_settings(tmp_path)
    _, used = _build_contents("pergunta", [make_source("missing", media_key="uploads/missing.png")], [], "detailed", settings)
    assert used == []
...
def test_skipped_media_does_not_consume_source_number(tmp_path: Path):
    ...
    contents, used = _build_contents("pergunta", [make_source("missing", media_key="uploads/missing.png"), source], [], "detailed", settings)
    assert used == ["text"]
    assert "[Fonte 1]" in contents[1]
...
def test_no_usable_source_returns_insufficient_without_gemini(tmp_path: Path, monkeypatch):
    ...
    answer = generate_answer("pergunta", [make_source("missing", media_key="uploads/missing.png")], [], "detailed", settings)
...
def test_api_returns_only_sources_used_by_generation(monkeypatch, tmp_path: Path):
    ...
    settings = make_settings(tmp_path)
    sources = [make_source("a", content_modality="text", chunk_text="a", file_type="text"), make_source("b", content_modality="text", chunk_text="b", file_type="text")]
...
def test_api_preserves_media_url_for_used_source(monkeypatch, tmp_path: Path):
    ...
    result = server._run_query(QueryRequest(question="pergunta"))
    assert result["sources"][0].media_url == "/uploads/media.png"
